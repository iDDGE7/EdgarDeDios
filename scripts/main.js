...
var scrollEndIntoBackground =
    containerAll.offsetTop +
    courtineBackground.offsetHeight -
    screen.height * 0.4;
var scrollInitIntoBackground =
    courtineBackground.scrollTop + screen.height * 0.7;

function backgroundCourtineMove() {
    if (
        posScroll > scrollInitIntoBackground &&
        posScroll < scrollEndIntoBackground
    ) {
...
function hiddenTitleBackgroundMobile() {
    if (screen.width < 750 || window.innerWidth < 750) {
        if (
            posScroll > window.innerHeight - window.innerHeight * 0.2 &&
            posScroll <
            window.innerHeight +
            courtineBackground.offsetHeight -
            courtineBackground.offsetHeight * 0.3
        ) {
        ...
        if (
            posScroll > scrollInitIntoBackground &&
            posScroll < scrollEndIntoBackground - 300
        ) {
...
function barsSize() {
    if (screen.width < 750 || window.innerWidth < 750) {
        if (
            posScroll > window.innerHeight - window.innerHeight * 0.2 &&
            posScroll < window.innerHeight + containerG1.offsetHeight - 200
        ) {
            increaseBarsWEB();
        } else {
            var percent = "12%";
        ...
        if (
            posScroll >
            window.innerHeight + containerG1.offsetHeight - screen.height * 0.5 &&
            posScroll <
            window.innerHeight +
            containerG1.offsetHeight +
            containerG2.offsetHeight -
            200
        ) {
            increaseBarsPROGRA();
        } else {
